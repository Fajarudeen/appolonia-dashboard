...
const EditDepartment = ({ deptData }: any) => {
  ...
  const {
    ...
    defaultValues: {
      department: deptData.department.department,
      section: deptData.department.section
    }
  })

  const onSubmit = async (values: TypeEmpSchema) => {
    try {
      const response = await departmentApi.putDepartment(router.query.id, values)
...
export async function getServerSideProps({ query }: any) {
  const deptData = await departmentApi.getDepartmentById(query.id)

  return { props: { deptData: deptData.data } }
}
